...
def apply_flat_dist_call(bucket, call, extra_args=None):
    ...
    if call is dist.all_reduce:
        ...
        # flattened already
        # apply hadamard transform if possible
        #print("Doing all reduce")
        t_dim = len(tensor)
        print(t_dim)
        if self.hadamard == 1:
            tensor = random_hadamard_encode(tensor, dim, prng=sgen)
            print(f"Using hadamard. Transformed tensor: {tensor}")

        # drop individual elements in gradient
        print("Dropping gradients!")
        ndropped = int(np.round(self.dc * tensor.numel()))
        dropped_idx = torch.randperm(dim)[:ndropped]
        tensor[dropped_idx] = 0.0
    ...
    if call is dist.all_reduce:
        # do inverse hadamard transform if possible
        if self.hadamard == 1:
            print("Doing inverse hadamard!")
...
class DistributedDataParallel(Module):
    ...
    def allreduce_bucket(self, bucket, bucket_idx, force_default_stream):
        tensor = flatten(bucket) # tensor is flattened
